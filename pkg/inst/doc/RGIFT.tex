...
Question 3 is an example of a Multiple Choice question where there is a
right answer which gives full mark, another not so good answer (which
gives a 50\%) and wrong answers (with a penalty). Note how now the
Multiple Answer flag is set (\code{MAnswer=TRUE}). As one of the answers
provides the full mark, Moodle only allows a single answer to be chosen.
...
\end{Schunk}
\noindent
Note that the previous text has been written in an environment with a
...
Sometimes questions require the use of images that display data or summary
results. In principle, this images should be among the files uploaded to
...
A convenient approach is to define an image repository and create all images
in JPEG or PNG format. Then the images can be uploaded to the image repository
and the complete link included in the question text. A simple example
...
> qtxt<-paste("[html]What\'s the histogram for a Binomial variable with n=10 and prob=0.25?\n<p>\n<img src=\'", repos, imgfile, "\'>\n<p>", sep="")
> #Print question
> sink(file="imgqtion.txt", type="output")
> GIFTMC(qtxt, c("(a)", "(b)", "(c)", "(d)"), rightans=1, wwrong="-33.333")
\end{Sinput}
\begin{Soutput}
[html]What's the histogram for a Binomial variable with n=10 and prob=0.25?
<p>
<img src='http://www.uclm.es/profesorado/vgomez/imgtests/q1.png'>
<p>
{
=(a)
~%-33.333%(b)
...
Sometimes equations need to be included in the question text or in the answers.
...
> GIFTMC("[html]The expectation of a random variable which is Binomial with $$n=10$$ and $$\\pi=0.7$$ is ", c("0.7", "1", "7", "0.07"), rightans=3)
\end{Sinput}
\begin{Soutput}
[html]The expectation of a random variable which is Binomial with $$n=10$$ and $$\pi=0.7$$ is 
{
~%0%0.7
~%0%1
=7
...
\section{Questionnaires in Moodle}

This code will produce a set of different questions which will possibly bleong
to different categories. The use of categories is encouraged since it allows
questions to be grouped into different topics. Moodle allows some customization
on the group of questions. 
...
set so that the student is assigned that number of questions at random among
all the questions in that section. Altogether, the student's exam will have
a fixed number of questions which come from the different categories at random 
as specified.

Regarding the output given to the student, the right answers and final score 
can be shown once the questionnaire has been finished. However, in certain
cases it may be better not to show the right answer immediately but
after the end of the exam (to make sure that all students have finished).
This is helpful to design questionnaire that include questions about different
...
using their university username and password.
...
This work has been partly funded by **proyecto innovación docente**
...
Industrial Engineering in the campus of Albacete (UCLM). These practicals are
organised in seven two-hour sessions. Sessions one to six are used to train the
students in the uses of \RR for descriptive and inferential statistics. In
these sessions the last half hour is used to answer some questionnaire about
the contects of the practical. These questionnaires is made of 15 different
questions which are randomly selected from the pool of question for that
practical. Each group of questions is assigned to a category depending on the
practical and, sometimes, the topic (for example, linear regression or ANOVA).
The students where allowed to repeat the test up to three times and the highest
score was kept. The students where given the right answer and the final mark
after they completed each test.

The last session is an overall exam of the course practicals. Here, the
students have the full two hours to complete the tests . As this exam needs to
...
the actual list of right ans wrong answer was available after all the students
...
In particular, for questionnaires the complete list of answer, socres, etc.
is available so that the lecturers can provide a complete analysis of the
responses. This is particularly interesting in order to find what parts of
the syllabus the students have not understood well or particularly 
difficult questions.


%\verbatiminput{test_exam.R}
** ICNLUDE R CODE TO PRODUCE THE TESTS **
